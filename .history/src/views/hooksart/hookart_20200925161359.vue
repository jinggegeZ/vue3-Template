<template>
  <div>
    ...
    <div class="d-f">
      ...
      <div class="m-r-10">
        <a-button type="danger" @click="setup(100)">
          设置
        </a-button>
      </div>
      <div class="m-r-10">
      ...
      <div class="m-r-10">
        <a-button type="primary" @click="delayPlus">
          延时加
        </a-button>
      </div>
      <div class="m-r-10">
        <a-button type="danger" @click="delayRed">
          延时减
        </a-button>
      </div>
      <div class="m-r-10">
        <a-button type="primary" @click="stepInc(5)">
          数量加
        </a-button>
      </div>
      <div class="m-r-10">
        <a-button type="danger" @click="stepRed(5)">
          数量减
        </a-button>
      </div>
      <div>
        当前值：<span class="contsize">{{ count }}</span>
      </div>
    </div>
    <br />
    <h2>本地存储</h2>
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    const add = () => {
      setMessage("user", data.name);
      data.title = getMessage("user");
    };
    const clear = () => {
      clearMessage("user");
      data.title = getMessage("user");
